...
def load_data_penguins(category_name):
    ...
    X_train, X_test, y_train, y_test = remove_nans(X_train, X_test, y_train, y_test)
...
def normalize(X, upper_bound=255):
    return X / upper_bound


def remove_nans(X_train, X_test, y_train, y_test):
    indices_training = np.unique(np.where(~np.isnan(X_train))[0])
    indices_test = np.unique(np.where(~np.isnan(X_test))[0])
    return X_train[indices_training], X_test[indices_test], y_train[indices_training], y_test[indices_test]
